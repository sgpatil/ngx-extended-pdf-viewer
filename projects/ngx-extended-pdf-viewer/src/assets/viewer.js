  ...
  /******/
    ...
    function getViewerConfiguration() {
      return {
        ...
        findBar: {
          ...
          findPageNumber: document.getElementById('findPageNumber'),
    ...
    class PDFFindBar {
      constructor(options, eventBus = (0, _ui_utils.getGlobalEventBus)(), l10n = _ui_utils.NullL10n) {
        ...
        this.findPageNumber = options.findPageNumber || null;
      ...
      dispatchEvent(type, findPrev) {
        this.eventBus.dispatch('find', {
          ...
          pageNumber: this.findPageNumber.value,
          phraseSearch: true,
    ...
    class PDFFindController {
      ...
      _calculateRegxMatch(query, pageIndex, pageContent, entireWord) {
        const matches = [];
        ...
        //let regx3 = new RegExp(strArray[0].trim()+"(.\\s*)?"+strArray[1].trim()+"(.\\s*)?"+strArray[2].trim()+"(.\\s*)?"+strArray[3].trim()+"(.\\s*)?"+strArray[4].trim()+"(.*)"+strArray[strArrayLength-4].trim()+"(.\\s*)?"+strArray[strArrayLength-3].trim()+"(.\\s*)?"+strArray[strArrayLength-2].trim()+"(.\\s*)?"+strArray[strArrayLength-1].trim(),"igm");

        // First 5 words
        let regx1 = new RegExp(strArray[0].trim() + "(.\\s*)?" + strArray[1].trim() + "(.\\s*)?" + strArray[2].trim() + "(.\\s*)?" + strArray[3].trim() + "(.\\s*)?" + strArray[4].trim(), ""); // igm

        // last 5 words
        let regx2 = new RegExp(strArray[strArrayLength - 4].trim() + "(.\\s*)?" + strArray[strArrayLength - 3].trim() + "(.\\s*)?" + strArray[strArrayLength - 2].trim() + "(.\\s*)?" + strArray[strArrayLength - 1].trim(), ""); //i

        let firstFewWordsFound = regx1.exec(pageContent);
        
        if (firstFewWordsFound && firstFewWordsFound.index !== -1) {
          this.fIndex = firstFewWordsFound.index;
          this.fPage = pageIndex;
        }


        let lastFewWordsFound = regx2.exec(pageContent);

        //console.log("Page :", pageIndex, "Findex:", firstFewWordsFound, "Lindex:", lastFewWordsFound, "QLength:", queryLen);
        //  console.log("lastFewWordsFound", lastFewWordsFound, "page", pageIndex);
        if (lastFewWordsFound && lastFewWordsFound.index !== -1) {
          this.lIndex = lastFewWordsFound.index;
          this.lPage = pageIndex;
          // console.log("matched", lastFewWordsFound);
          //matches.push(lastFewWordsFound.index);
        }



        // if (firstFewWordsFound && lastFewWordsFound) {
        //   console.log("page:", this._state.pageNumber); 
        //   // check both indexs are in range
        //   // console.log("lastFewWordsFound index", lastFewWordsFound.index,  firstFewWordsFound.index+queryLen);
        //   if (lastFewWordsFound.index < firstFewWordsFound.index + queryLen + 40) {
        //     //console.log("Page :", pageIndex, "Findex:", firstFewWordsFound.index, "Lindex:", lastFewWordsFound.index, "QLength:", queryLen);
        //     matches.push(firstFewWordsFound.index);
        //   }
        // }else{
          
        // }

        //if(this._state.pageNumber -1 == pageIndex){
          if(firstFewWordsFound && firstFewWordsFound.index){
            matches.push(firstFewWordsFound.index);
          }else{
           // matches.push(1);
      ...
      _calculateMatch(pageIndex) {
        ...
        if (phraseSearch) {
          this._calculateRegxMatch(query, pageIndex, pageContent, entireWord);
        } else {
